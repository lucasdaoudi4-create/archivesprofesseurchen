...
const PLAFOND_JPEG = 1600;

function echelle(v: Visuel, ext: "avif" | "webp" | "jpg"): readonly Derivee[] {
  return ext === "jpg" ? v.derivees.filter((d) => d.l <= PLAFOND_JPEG) : v.derivees;
}

/** Construit le chemin d'une dérivée dans un encodage donné. */
export function chemin(v: Visuel, d: Derivee, ext: "avif" | "webp" | "jpg"): string {
  return `${RACINE}${v.base}-${d.l}x${d.h}-v01.${ext}`;
}

/** Construit l'attribut `srcset` complet d'un encodage. */
export function jeuSources(v: Visuel, ext: "avif" | "webp" | "jpg"): string {
  return echelle(v, ext)
    .map((d) => `${chemin(v, d, ext)} ${d.l}w`)
    .join(", ");
}

/** La dérivée de tête : la plus grande, celle qui pose le rapport de forme. */
export function repli(v: Visuel): Derivee {
...
/* ── LE REPLI DE L'ATTRIBUT `src` ──────────────────────────────────────────
   ...
   de l'époque.                                                             */
const CIBLE_SECOURS = 1280;

export function secours(v: Visuel): Derivee {
  return v.derivees.reduce((meilleure, d) =>
    Math.abs(d.l - CIBLE_SECOURS) < Math.abs(meilleure.l - CIBLE_SECOURS) ? d : meilleure,
  );
}

/** Le ratio de la plus grande dérivée, à poser en CSS pour réserver la place. */
export function ratio(v: Visuel): number {
  const d = repli(v);
  return d.l / d.h;
}
...
export const TAILLES_SCENE = `(min-width: ${BP_PAILLASSE}) max(49vw, 853px), 100vw`;
...
export const TAILLES_NOTICE = `(min-width: ${W_WRAP_PLEIN}) 542px, (min-width: ${BP_PAILLASSE}) 44vw, 92vw`;
...
export const TAILLES_PORTRAIT = `(min-width: ${W_WRAP_PLEIN}) 437px, (min-width: ${BP_PAILLASSE}) 36vw, 24rem`;
...
export const TAILLES_PLANCHE_404 = "50vw";

/* L'affiche du lecteur vidéo : toute la largeur du `.wrap`. */
export const TAILLES_AFFICHE = `(min-width: ${W_WRAP_PLEIN}) 1140px, 92vw`;
...
export const plateauLarge: Visuel = {
  ...
  alt:
    "Le narrateur, de dos, travaille à la paillasse ovale du laboratoire, " +
    "sous l'enseigne au néon des Archives.",
  aplat: "var(--lqip-dec01)",
};

export const plateauHaut: Visuel = {
  base: "arc-dec-plateau-principal",
  derivees: [
    { l: 640, h: 480 },
    { l: 960, h: 720 },
...
export const narrateurPortrait: Visuel = {
  ...
  alt:
    "Le narrateur en blouse blanche, de trois quarts, dans le laboratoire, " +
    "l'enseigne au néon derrière lui.",
  aplat: "var(--lqip-nar)",
};

export const narrateurPlanDeFace: Visuel = {
  ...
  alt:
    "Le narrateur, de face derrière la paillasse ovale, un carnet ouvert " +
